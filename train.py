...
gen_x2yd = rcan.RCAB().to(device)
upsample_net = rcan.RCAB().to(device)
...
for epoch in range(training_parameteres['epochs']):
    ...
    try:
        ...
        for idx, sample in enumerate(batch):
            ...
            real_yd = F.interpolate(real_y, size=real_x.shape, mode='bicubic') # downsample the high resolution image
